...
const STORAGE_KEY = "hallDeGuildeAdminData_v1";
...
function getAllData() {
  return {
    ...
    campaigns,
    loreSections,
    worldMapDataUrl: typeof currentWorldMapDataUrl === "string" ? currentWorldMapDataUrl : ""
  };
}
...
function saveAllData() {
  try {
    ...
    console.error("Erreur de sauvegarde des données du Hall de guilde :", e);
  }
}
...
function loadAllData() {
  try {
    ...
    if (Array.isArray(data.loreSections)) {
      loreSections = data.loreSections;
    }
    if (data.worldMapDataUrl && typeof data.worldMapDataUrl === "string") {
      currentWorldMapDataUrl = data.worldMapDataUrl;
      if (typeof worldMapImage !== "undefined" && worldMapImage) {
        worldMapImage.src = currentWorldMapDataUrl;
      }
    }
  } catch (e) {
    console.error("Erreur de chargement des données du Hall de guilde :", e);
  }
}
    ...
    card.innerHTML = `
      <div class="character-main">
        <div class="character-portrait-wrapper">
          <div class="${portraitClass} ${tierClass}" style="${portraitStyle}">
            ${ch.portraitUrl ? "" : "🖼️"}
          </div>
        </div>
      ...
      <div class="character-extra">
        ...
        <div class="character-lore">
          ${
            ch.lore && ch.lore.trim()
              ? ch.lore
              : "<em>Aucune histoire renseignée pour le moment.</em>"
          }
        </div>
      </div>
    `;
...
if (createForm) {
  createForm.addEventListener("submit", (event) => {
    ...
    const lore = document.getElementById("char-lore").value.trim();
...
function startEditCharacter(id) {
  ...
  const loreInput = document.getElementById("char-lore");
  const xpInput = document.getElementById("char-xp");
  ...
  if (loreInput) loreInput.value = char.lore || "";
...
function exportDataToFile() {
  try {
    ...
    a.href = url;
    a.download = "hall-de-guilde-data.json";
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  } catch (e) {
    console.error("Erreur lors de l’export des données du Hall de guilde :", e);
  }
}
